...
function ChatItem({ ch, base, label, currentUserId }) {
  ...
  const badgesAndTitle = (
    <span className="min-w-0 flex-1">
      ...
      {ch.description && (
        <span className="mt-0.5 block text-sm text-stone-500">{ch.description}</span>
      )}
      {created && (
        <span className="mt-0.5 block text-xs text-stone-400">Started {created}</span>
      )}
    </span>
  );
    ...
    <li className="rounded-xl border border-stone-200 bg-white shadow-sm overflow-hidden">
      {canOpen ? (
        <Link
          ...
          className="flex w-full items-start gap-3 px-4 py-4 transition hover:bg-stone-50"
        >
          {badgesAndTitle}
          <span className="mt-0.5 shrink-0 text-stone-300 transition-colors group-hover:text-stone-500" aria-hidden>
